...
int main() {
	...
	cout << " center row " << ((Mat)center).row(0) << endl << endl; //[300]만 출력 

																	//Mat클래스는 행렬을 지정해주지 않고 default 상태일 때는 
																	//[300;
																	// 150;
																	//  1]; 이렇게 저장 된다.	

	cout << " center col " << ((Mat)center).col(0) << endl << endl; //[300;
																	// 150;
																	//  1]; 출력 

	t1.row(0).col(2) += ((Mat)center).row(0);						//그렇기 때문에 연산을 할 때 row(n)를 해야 300, 150, 1에 접근이 가능하다.
	t1.row(1).col(2) += ((Mat)center).row(1);

	t2.row(0).col(2) -= ((Mat)center).row(0);
	t2.row(1).col(2) -= ((Mat)center).row(1);
	
	
	cout << " 단위 행렬 + 300,150 "<< t1 << endl;
	cout << " 단위 행렬 + -300, -150" << t2 << endl;
	...
	/*t1.row(2).col(1) = t1.row(2).col(1) + ((Mat)center).row(1).col(1);
	t1.row(2).col(1) = t1.row(2).col(1) + ((Mat)center).row(1).col(1);*/
	...
	gemm(t1, m, alpha,noArray(), beta,m);				//행렬은 순서위치에 따라 연산 결과가 달라지기 때문에 순서를 주의 한다. 
	
	gemm(m, t2, alpha, noArray(), beta, m);
